...
const useResizable = (): void => {
    const startX = ref(0) // 鼠标开始点击时的位置
    const startWidth = ref(0) // 记录鼠标按下那一刻 左边 .scalable 的宽度
    // 获取由 leftClass 定义的类名的元素的宽度尺寸

    const getWidth = () => {
        const leftDivElement: HTMLDivElement = document.querySelector(leftClass)!
        const leftDivElementWidth: string = leftDivElement.style.width
        return parseInt(leftDivElementWidth, 10)
    }
    ...
    const onDrag = (e: MouseEvent) => {
        ...
        ;(document.querySelector(leftClass) as HTMLDivElement).style.width = newWidth + "px"
        // 右边区域距离左边=左边宽度让出来(2处)
        // document.querySelector(rightClass).style.marginLeft = newWidth + 'px'
    }
    const stopDrag = () => {
        // 把 数据保存到 localStorge
        localStorage.setItem(resizable_width, String(getWidth()))
        localStorage.setItem(is_narrow, String(isNarrow.value))
    ...
    onMounted(() => {
        ...
        startWidth.value = (localStorage.getItem(resizable_width) || getWidth()) as number
        // 初始化时从 localStorge 中获取 isNarrow
        isNarrow.value = JSON.parse(localStorage.getItem(is_narrow) as string) || false
        // 获取初始的宽度值
        ;(document.querySelector(leftClass)! as HTMLElement).style.width = startWidth.value + "px"
        // 右边区域距离左边=左边宽度让出来(2处)
        // document.querySelector(rightClass).style.marginLeft = startWidth.value + 'px'
        //! 1, 鼠标按下时触发
        ;(document.querySelector(leftClass)! as HTMLElement).addEventListener("mousedown", startDrag)
        ...
        const onresize = function() {
            ...
            if (screenWidth < 768) {
                // 当处于窄屏状态时
                (document.querySelector(leftClass) as HTMLElement).style.width = minResizableWidth + "px"
                isNarrow.value = true
            } else {
                // 宽屏时, 取消 narrow类, 这个时候, 如果这个时候左侧边栏如果小于断点, 仍然会
                // 按照大屏的显示, 这是不合理的, 应该是如果左侧小于断点, 还是按照小屏显示
                (document.querySelector(leftClass) as HTMLElement).style.width =
                    localStorage.getItem(resizable_width) + "px"
